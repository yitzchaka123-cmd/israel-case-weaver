...
I. **FICTIONAL BRAND KIT (REQUIRED for any in-world organisation)** — Real props look real because every page from the same office uses the same logo, wordmark and color. BEFORE drafting documents, identify each recurring in-world organisation in the case (e.g. "Precinct 14", "St. Marlow Forensics Lab", "The Daily Ledger", "Kessler & Sons Couriers", "County Morgue", "Ministry of Transport"). For EACH org, draft a one-paragraph **brand kit** describing an INVENTED logo concept (shape/crest/wordmark), color palette, typography, and era cue — then PREPEND that exact paragraph to the \`design_instructions\` of EVERY document issued by that org, e.g. \`BRAND KIT — Precinct 14: gold-trimmed navy shield with oak-leaf border, wordmark "PRECINCT 14 — METRO POLICE" in slab serif, ink-stamp navy + cream paper, 1970s municipal vibe. Reuse this exact lockup on every Precinct 14 document.\` Rules: (a) the three consistent families above inherit the precinct/police brand kit automatically; (b) standalone one-offs (lab report, autopsy, news clipping, courier receipt, bank statement, etc.) MUST embed their own org's brand kit paragraph in their \`design_instructions\`; (c) NEVER invent a real-world brand or imitate a real police seal, real news masthead, real government emblem, or any copyrighted mark — purely fictional only; (d) NEVER reuse the same fictional logo across two different in-world orgs.`;
})()}

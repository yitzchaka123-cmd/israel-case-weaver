...
SUSPECT POLICE BRIEFINGS (MANDATORY — COUNTED IN target_doc_count): The proposed document set MUST include exactly ONE "Police Briefing" document per suspect — including red herrings — with NO exceptions for case genre. Title each one in-language using the pattern "Police Briefing — <Suspect Name>" (Hebrew: "תדריך משטרתי — <שם>" / Arabic: "ملف الشرطة — <الاسم>" / Spanish: "Informe Policial — <Nombre>" etc.). Set \`doc_type\` to "Suspect profile" (or "Police briefing"). Each briefing MUST list the matching suspect's id in \`linked_suspect_ids\` so the UI pins the suspect's portrait into the document's first inline-image slot as the LOCKED ANCHOR (see ANCHOR PORTRAIT RULE below). These briefings COUNT toward \`target_doc_count\` like every other loose-pile document. When auditing the proposal, confirm: (a) one briefing per suspect row exists, (b) every briefing has \`linked_suspect_ids\` populated, (c) the total document count (Doc 0 + briefings + other evidence docs) is within ±5 of \`target_doc_count\`. If suspects exist and briefings are missing, REBALANCE the proposal — never ship a set without per-suspect briefings.

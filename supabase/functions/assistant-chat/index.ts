...
Phase 3 Structure: suspects, clue sequence, red herrings, deduction logic, and the sealed task-envelope plan. Output fits the node canvas. IMPORTANT GAME-FLOW MODEL: Envelopes are SEALED TASK GATES — they do NOT distribute documents in batches. All evidence documents live loose in the box from the very start; the player has access to every document immediately. Envelopes only hold a short task / reveal / instruction the player reads when they reach the matching beat in the case (e.g. "Open envelope 2 once you've narrowed it down to two suspects"). Envelope #1 is the mission briefing (opened first, NEVER mentions Doc 0 or any specific document — only refers to "the case file" generically). The final envelope contains the accusation form / solution reveal. When you plan envelopes you must reason about each envelope's OPENING TRIGGER (the case beat that unlocks it) and its PAYLOAD (task, reveal, or instruction). NARRATION RULE: When you describe an envelope to the user — in chat, in summaries, or in node descriptions — NEVER say it "contains" or "holds" clues / documents / evidence. Only the final envelope physically contains the accusation form. For every other envelope, describe (a) the task the player reads when they open it, and (b) which loose-pile clues the player should ALREADY be holding when they reach that beat (phrase as "relevant clues" or "the player should already have figured out…", never "inside the envelope"). SINGLE-ENVELOPE HELP RULE: When the user asks for help drafting a SPECIFIC envelope ("help me with envelope #N", "draft envelope 1", "fill in the briefing envelope", or any per-slot request originating from the Envelopes panel) — and especially for envelope #1 (the mission briefing) — you MUST in the SAME turn call \`update_envelope\` with the proposed \`label\`, \`task\`, and \`design_instructions\` for that specific slot. Never just chat about it without writing — silent no-ops are a critical failure. If the envelope row does not yet exist for that slot, propose the content and immediately call \`update_envelope\` (the backend creates the row) before asking the user to refine.
...
CAST SIZE GUIDANCE (HARD RULE): When you create the initial suspect roster, target counts by difficulty: easy → exactly 5 suspects (NEVER fewer), medium → 6–7, hard → 7–10. Use these as strong defaults; deviate only if the user explicitly says a different number. CAST SIZE LOCK: once the roster exists in the \`suspects\` table, it IS the cast — do NOT re-ask "keep N / expand to M" via \`propose_options\` more than once per project. EXCEPTION: if the project's \`difficulty\` is easy and \`suspects.length < 5\`, OR medium and \`suspects.length < 6\`, OR hard and \`suspects.length < 7\`, you MAY (once per project) proactively offer "Add 1–2 more suspects?" via \`propose_options\` with concrete name+role suggestions. Otherwise, only act on suspect changes when the user explicitly asks ("add a suspect X", "remove Y") — then call \`add_suspect\` / \`update_suspect\` / \`delete_suspect\`.
PORTRAIT PROMPT RULE (HARD): Every \`add_suspect\` call MUST include a \`thumbnail_prompt\` (40–80 words, photoreal portrait brief: apparent age, ethnicity/build, hair, distinctive features, wardrobe, lighting/mood — derived from the suspect's name + summary + role_in_case). The Suspects panel auto-generates the portrait the moment that field lands. If you ever notice an existing suspect with a missing portrait (no \`thumbnail_url\` AND no \`thumbnail_prompt\`), use \`update_suspect\` to set a \`thumbnail_prompt\` for it — never leave a suspect without a portrait brief.
...
C. **Doc 0 + the rest in the SAME turn (CRITICAL — applies to "draft all docs" / "draft everything" / "draft all the documents" / any phrasing that asks for the full document set in one go)** — When the user approves a multi-doc proposal — even a 40-doc one — you fire ALL batches in this same turn with NO confirmation question. Asking "ready?", "say go and I'll start", "starting with docs 1-3?", "before I fire those batches", "tool limit hit, tell me go", "quick confirmation", or any variant is a CRITICAL FAILURE. The 3-doc per-call cap is an internal mechanic — NEVER surface it to the user as a reason to pause; just chain the batches silently. Doc 0 (contents inventory) requires special handling and MUST go through \`add_document\`. The order is:
   ...
   **C.1 — BATCH GROUPING (consistent visual sets first):** When you split the proposal across \`add_documents\` calls, GROUP docs that share a visual template into the SAME 3-doc batches BEFORE the standalone one-offs. Templated families are STRICTLY LIMITED to THREE families: (1) Interrogation Transcripts/Logs, (2) Police Reports / Police Briefings, and (3) Police Intake Reports. EVERY other document — forensic/autopsy reports, witness statements, suspect file pages, lab reports, evidence intake forms, maps, photos, news clippings, letters, tickets, receipts, diagrams, ledgers, etc. — is a STANDALONE one-off and MUST get its own unique paper, layout, fonts, and stamps (visual variety is part of the player's experience). Standalone one-offs go in the trailing batches. Mention the grouping in ONE short line at the end (e.g. "Drafted all 40 — grouped 6 interrogation logs, 5 police reports and 1 intake report into consistent sets; the rest are standalone.").
...
H. **CONSISTENT-SET DOC IMAGES (THREE FAMILIES ONLY)** — the ONLY document families that should ever go through \`generate_consistent_document_set\` are: (1) Interrogation Transcripts/Logs, (2) Police Reports / Police Briefings, (3) Police Intake Reports. For these three families, do NOT call \`generate_document_assets\` per-doc; after the docs exist, call \`generate_consistent_document_set\` ONCE per family with all their ids in one array (max 8 per call; chain calls if more). All OTHER documents (forensic reports, witness statements, lab reports, maps, photos, letters, tickets, receipts, etc.) MUST be generated as standalone one-offs via the normal bulk pipeline so each gets a distinct visual identity — do NOT bundle them into consistent sets even if there are several of the same kind. **GENERATION ORDER (for "generate all" runs):** FIRST issue the \`generate_consistent_document_set\` call(s) for the three templated families, THEN issue \`bulk_generate_documents\` for the standalone one-offs. This way the locked sets render before the random standalones land. Suspect portraits are passed pixel-perfect from each suspect's existing thumbnail file, so faces stay identical across the consistent sets. Forensic reports are STANDALONE but MUST still include a clinical body-injury diagram as one of the inline images (use \`add_document_inline_images\` with a slot labeled "Body injury diagram" describing front+back anatomical silhouettes with numbered injury markers).

I. **FICTIONAL BRAND KIT (REQUIRED for any in-world organisation)** — Real props look real because every page from the same office uses the same logo, wordmark and color. BEFORE drafting documents, identify each recurring in-world organisation in the case (e.g. "Precinct 14", "St. Marlow Forensics Lab", "The Daily Ledger", "Kessler & Sons Couriers", "County Morgue", "Ministry of Transport"). For EACH org, draft a one-paragraph **brand kit** describing an INVENTED logo concept (shape/crest/wordmark), color palette, typography, and era cue — then PREPEND that exact paragraph to the \`design_instructions\` of EVERY document issued by that org, e.g. \`BRAND KIT — Precinct 14: gold-trimmed navy shield with oak-leaf border, wordmark "PRECINCT 14 — METRO POLICE" in slab serif, ink-stamp navy + cream paper, 1970s municipal vibe. Reuse this exact lockup on every Precinct 14 document.\` Rules: (a) the three consistent families above inherit the precinct/police brand kit automatically; (b) standalone one-offs (lab report, autopsy, news clipping, courier receipt, bank statement, etc.) MUST embed their own org's brand kit paragraph in their \`design_instructions\`; (c) NEVER invent a real-world brand or imitate a real police seal, real news masthead, real government emblem, or any copyrighted mark — purely fictional only; (d) NEVER reuse the same fictional logo across two different in-world orgs; (e) **ENVELOPES ARE THE EXCEPTION** — sealed task envelope inserts are publisher-branded, not in-world. They carry the workspace COMPANY LOGO (real publisher branding) in the top zone, NEVER an invented in-world precinct/dispatch crest. Do not attach a fictional brand kit to any envelope's \`design_instructions\`.`;
})()}
...
async function processConversation(
  ...
  // could flip its placeholder/run row. CRITICAL: apply an age floor of 90s
  // so we don't kill the run we ourselves just inserted (background mode
  // creates the assistant_runs row in the outer handler before invoking this
  // function — without an age guard, this sweep would mark the in-flight run
  // as "auto_closed_zombie" and the client would treat the new turn as failed.)
  const zombieCutoff = new Date(Date.now() - 90 * 1000).toISOString();

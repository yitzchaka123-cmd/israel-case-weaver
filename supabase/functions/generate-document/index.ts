...
async function loadDoc0InventoryContext(supa: any, projectId: string) {
  ...
  return {
    ...
    text: [
      ...
      ``,
      `🚨 ABSOLUTE FORMAT — DOC 0 IS A BARE INVENTORY LIST. NOTHING ELSE.`,
      `- Output ONLY two sections: (1) "Documents" and (2) "Envelopes".`,
      `- Documents section: ONE LINE per document, formatted EXACTLY as "#N — Short Name" (in the project language). NO doc_type, NO print size, NO purpose, NO description, NO grouping headers, NO "visual feel", NO commentary. Just number + dash + short title.`,
      `- Envelopes section: ONE LINE per envelope, formatted EXACTLY as "Envelope N — Short Label" (translated, e.g. "מעטפה 2 — מעטפת המשימה"). NO task text, NO opening trigger, NO "open after…" hints, NO payload — those are spoilers.`,
      `- Every suspect must already appear in the Documents list as their own "Police Briefing — <Suspect Name>" entry (planned by the document set). Do NOT add a separate "Suspects" section in Doc 0.`,
      `- Do NOT mention the solution, the killer, the twist, the red herring, or any deduction.`,
      `- Doc 0 is a contents page on a board game. Keep it minimal. Anything beyond "#N — Name" / "Envelope N — Label" lines is a failure.`,
      ``,
      `DOCUMENTS TO LIST (use these EXACT numbers and titles — print only "#N — Title", drop everything in parentheses):`,
      docNodes.map((d: any) => `- #${d.docNumber} — ${d.title}   [REFERENCE ONLY, DO NOT PRINT: ${d.docType}, ${d.printSize} · ${d.purpose}]`).join("\n") || "(none)",
      `\nENVELOPES TO LIST (use these EXACT numbers and labels — print only "Envelope N — Label"):`,
      (envelopes ?? []).map((e: any) => `- Envelope ${e.number} — ${e.label ?? `Envelope ${e.number}`}   [REFERENCE ONLY, DO NOT PRINT: ${e.task ?? "(no task)"}]`).join("\n") || "(none)",
      `\nSUSPECTS (REFERENCE ONLY — every suspect must already exist as a "Police Briefing — <Name>" entry in the Documents list above; do NOT add a separate suspects section to Doc 0):`,
      (suspects ?? []).map((s: any) => `- ${s.name}${s.role_in_case ? ` — ${s.role_in_case}` : ""}`).join("\n") || "(none)",
      `\nEXISTING DOCUMENT ROWS (for status only; ignore if Final Flow differs):`,
...
- REALISM IS REASONED, NOT TEMPLATED: any realism details you mention must come from what a real example of THIS document type, in THIS era and setting, would actually have on it (e.g. a police booking sheet → mugshot + ten-print card + booking number; a school detention slip → student name + period + teacher initials + three-hole punch). Do NOT default to coffee stains, water rings, fold lines, or generic "smudged ink" — those are massively overused. Coffee/water stains are banned by default; only include one if the story specifically puts this document somewhere it would have gotten stained, and never on more than one document in the whole game.`;
    ...
    if (mode === "image") {
      ...
      const imgPrompt = doc0
        ...
        : [
            ...
            ``,
            `BRANDING / LOGO (REQUIRED for any in-world institutional document):`,
            `- If this document originates from any organisation — police precinct, crime lab, hospital, morgue, newspaper, courier company, bank, school, government office, private firm, etc. — it MUST carry a small INVENTED FICTIONAL logo / crest / wordmark / letterhead lockup at the top, and a matching footer mark (file code, dept name, address line) where appropriate.`,
            `- The logo must be 100% fictional and period/setting-appropriate. Never copy or imitate any real-world brand, real police seal, real news masthead, real government emblem, or any copyrighted mark.`,
            `- If the design instructions above include a "BRAND KIT — <Org>" paragraph, render EXACTLY that lockup (same shape, wordmark, color, type) so every document from that organisation looks like it came from the same place.`,
            ``,
            `RULES:`,
            ...
            `- Do NOT use logos of REAL companies, real police departments, real news outlets, or any real-world emblem. Invented fictional logos for the in-world agencies are REQUIRED, not forbidden.`,

...
- Envelopes #2..#${count - 1} (Stage Recap → Follow-up Task): Open with a recap that is SCOPED STRICTLY TO THE PREVIOUS ENVELOPE'S TASK AND NOTHING ELSE. Use the equivalent of "By now, you've worked through [the PREVIOUS envelope's task topic] — and you've probably worked out that…" Two paragraphs that go DEEP into that one previous task only: what the detective was asked to do, and the in-world conclusion they should now hold in mind from doing it. HARD LIMITS — the recap MUST NOT: (a) summarise the cumulative state of the case, (b) recap or hint at any earlier envelope's task (anything before envelope #N-1), (c) list other things the detective has figured out so far across the whole investigation, (d) pre-summarise or foreshadow beats that come AFTER the previous task, (e) name a specific document, (f) reveal the culprit, motive, method, red herring, or decisive clue. Refer to suspects by name only when the PREVIOUS task was about them. Go deeper into that single beat — do not go wider.
...
- ONE clear, straightforward investigative goal in the world, INVENTED FRESH for THIS case's beat — not template phrasing. Examples of the SHAPE only (do NOT copy these words): "Find out who is lying.", "Figure out who could have been at the scene at the time of the murder.", "Decide who actually had a reason to want him dead." Use the Logic Flow node and case context to write a goal specific to this case. The task must name WHAT the detective is investigating (a suspect interaction, a contradiction, a timeline window, a location, a relationship, a motive question — at the category level only, never naming a specific document or clue) AND the mental conclusion they should reach before opening the next envelope.
- TASK UNIQUENESS (LOCKED): Each envelope's red task line MUST target a DIFFERENT Logic Flow beat than every other envelope in the set. No two envelopes may share the same task verb + target. Before emitting the JSON, mentally diff each task against the others and rewrite any that overlap.
- FORBIDDEN GENERIC TASK PATTERNS (and their ${gameLanguage} equivalents — these describe OPENING an envelope, not doing investigative work; never ship them): "go through all the evidence in this envelope", "go through the evidence", "review what you have", "review the case file", "look at everything in this envelope", "find the next task", "find out the next task", "figure out the next step", "decide what to do next", "examine the case file", "study the documents", "open this envelope and continue", "see what's in here", "read everything carefully", "take stock of what you have". If the task could equally apply to any envelope in any case, it is wrong — rewrite it as a concrete, case-specific investigative goal anchored to this envelope's Logic Flow beat.
...
- "task": the FULL A4 in-character letter described above, in ${gameLanguage}, ${isRtl ? "RTL" : "LTR"}. ~380–560 words for #1..#${count - 1} (hard floor 340 — do NOT ship short), ~280–420 for the final. MUST follow the three-part A/B/C structure (Briefing-or-Recap → Your Task (red bold line, NO supporting prompt list) → Seal Instruction). Envelope #1 only adds a short general-tips block after the red task line. For env #1..#${count - 1}: strictly no specific-document, specific-clue, or solution references. Final envelope (#${count}) instead carries the four-beat cinematic verdict + reveal (cinematic close → red verdict line naming the culprit → reveal paragraph → sign-off + QR card call-out). NO-WRITING RULE applies everywhere — never tell the player to write, jot, note, list, chart, or fill in anything; use mental-tracking phrasing instead. The closing line is appended by the UI — do NOT include it.
            ...
            envelopes: {
              ...
              items: {
                ...
                properties: {
                  ...
                  label: { type: "string" },
                  task: { type: "string", description: "Full A4 in-character letter from the Case Officer to the Detective, ~380–560 words (hard floor 340) for env #1 and middle envelopes, ~280–420 for the final envelope. Three-part structure: PART A briefing (env #1, cinematic 2–4 sentence opener) or recap 'By now you've probably worked out…' (middle envelopes), at least 2 paragraphs ~180–280 words; PART B '<TASK_RED_LINE>Your task: ...</TASK_RED_LINE>' on its own line (rendered red bold by the design pass) and NOTHING after it (no supporting prompt list, no how-to-solve bullets) — EXCEPT envelope #1 which appends one short general spoiler-free tips block; PART C 2–3 line seal instruction telling the player to only open the next envelope when sure, plus in-character sign-off. Final envelope (#N) instead carries the four-beat cinematic verdict + reveal: BEAT 1 cinematic closing scene, BEAT 2 single bold red <TASK_RED_LINE>Verdict: {Culprit} killed {Victim}.</TASK_RED_LINE> line that explicitly names the culprit, BEAT 3 reveal paragraph (4–7 sentences) confirming culprit/method/motive from the Solution Summary, BEAT 4 sign-off + framed-QR-card call-out. NO-WRITING RULE: never tell the player to write/jot/note/list/chart/fill in anything — use mental-tracking phrasing. For envs #1..#N-1: strictly NEVER references specific document numbers/titles, specific clue mechanics, or the solution. Closing line is appended by UI — do not include it." },

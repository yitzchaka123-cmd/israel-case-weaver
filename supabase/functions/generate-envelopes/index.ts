...
FINAL ENVELOPE QR (CRITICAL — applies ONLY to envelope #${count}): The design_instructions MUST require a printed QR code in a clearly marked spot on the page (e.g. lower portion, framed by a thin border, with a short ${gameLanguage} caption like "Scan to view the official case-closure broadcast"). Treat the QR as a placeholder square in the mockup — the actual scannable QR is composited later by the app from the user-supplied YouTube URL.
            ...
            envelopes: {
              ...
              items: {
                ...
                properties: {
                  ...
                  label: { type: "string" },
                  task: { type: "string", description: "Full A4 in-character letter from the Case Officer to the Detective, ~450–700 words (hard floor 400) for env #1 and middle envelopes, ~200–320 for the final envelope. Three-part structure: PART A briefing (env #1, cinematic 2–4 sentence opener) or recap 'By now you've probably worked out…' (middle envelopes), at least 2 paragraphs ~180–280 words; PART B '<TASK_RED_LINE>Your task: ...</TASK_RED_LINE>' on its own line (rendered red bold by the design pass) + 3–5 general investigative prompts in normal body type; PART C 2–3 line seal instruction telling the player to only open the next envelope when sure, plus in-character sign-off. Final envelope (#N) instead carries solution confirmation + QR scan call-out (the answer is allowed there only). For envs #1..#N-1: strictly NEVER references specific document numbers/titles, specific clue mechanics, or the solution. Closing line is appended by UI — do not include it." },

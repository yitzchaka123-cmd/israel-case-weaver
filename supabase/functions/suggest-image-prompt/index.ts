...
const CATEGORY_GUIDANCE: Record<string, string> = {
  ...
  envelope: "A full A4 in-world page insert that will be placed inside a physical envelope — a regular era-appropriate briefing/recap/task sheet (e.g. typed memo on letterhead). Not an envelope cover: no flap, no wax seal, no mailer. Keep realism subtle and reasoned from the page type/era — never default to coffee stains, water rings, or generic fold lines.",
...
Require the brief to place the logo in the top letterhead/header area of the A4 page insert (top-center, top-left, or top-right — pick the spot that frames this page best). Logo height ≈ 5–9% of the page's longer side, with breathing room from stamps, file codes, and title text. Treat the logo as if printed onto the page (matte ink, period-correct registration), NOT a sticker, NOT a watermark, no drop shadows. Render the company name in small clean type beside or beneath the logo when supplied.`
    ...
    if (category === STRUCTURED_DOC || category === STRUCTURED_ENV) {
      ...
      const structuredSystem = [
        ...
        ``,
        isEnv
          ? `Envelope-slot rules: produce an A4 page insert, not the outside of an envelope. Design it as a regular era-appropriate briefing/recap/task page (a typed memo on letterhead is the safe default). It is fine — even preferred — for envelope inserts in the same set to share the same letterhead and visual language; they are a coherent series, not a sampler of document types. Keep realism subtle (2–3 details max) and reasoned from this page's type and era. NEVER default to coffee stains, water rings, fold lines, or generic "smudged ink" — and never repeat the same realism detail on multiple inserts. Never reveal the case solution.`
